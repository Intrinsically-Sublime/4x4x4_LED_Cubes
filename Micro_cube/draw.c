...
// Set a single voxel in the temporary cube buffer to OFF
...
char flipbyte (char byte)
{
	...
	if (CUBE_SIZE == 4) {
		...
		flop = ((0x1 & byte) << 3) | ((0x2 & byte) << 1) | ((0x4 & byte) >> 1) | ((0x8 & byte) >> 3);
	}
	return flop;
}
...
void tmp2cube (void)
{	
    memcpy(cube, fb, CUBE_BYTES); // copy the current fb into rendering buffer.
}

// Copies the contents of cube into fb (temp cube buffer) 
void cube2tmp (void)
{	
    memcpy(fb, cube, CUBE_BYTES); // copy the current cube into fb.
}
